...
class ShiftChangeRequestAssignment
{
    /**
     ...
     * @param  int[]  $lateShiftUserIds
     * @param  int[]  $nightShiftUserIds
     * @return ShiftChangeRequestAssignment
     *
     * @throws \InvalidArgumentException
     */
    public static function create(DateTimeImmutable $date, array $dayShiftUserIds, array $lateShiftUserIds, array $nightShiftUserIds)
    {
        if (count($nightShiftUserIds) < 2) {
            throw new \InvalidArgumentException('夜勤の人は2人以上必要です。');
        }

        $shiftChangeRequestAssignmentEntity = new ShiftChangeRequestAssignment(
            $dayShiftUserIds,
            $lateShiftUserIds,
            $nightShiftUserIds
        );
        ...
        $isBusinessDay = ! in_array($date->format('D'), $closedWeekDays)
        ...
        return $shiftChangeRequestAssignmentEntity;
    }
}
